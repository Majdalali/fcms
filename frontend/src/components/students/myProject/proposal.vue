    ...
    <v-container fluid>
      <v-row>
        ...
        >
          ...
          <div class="lowerDiv pt-10 w-4/5">
            <v-card
              class="vCard"
              variant="elevated"
              outlined
...
onMounted(async () => {
  ...
  try {
    ...
    try {
      ...
      console.log("Error fetching other files:", error);
    }
    ...
    errorMessage.value = "Error fetching info. Please try again later.";
  }
...
const deleteFile = async (autogeneratedName, dialogType) => {
  try {
    ...
    const response = await axios.delete(
      `${apiUrl}/deleteFile/${autogeneratedName}`,
      {
        headers: {
          Authorization: token,
          "Content-Type": "application/json",
