      ...
      <v-row>
        ...
        >
          <div class="pt-4 upperDiv">
            <h1 class="text-3xl font-medium title">Presentation</h1>
            <p class="text-base titleDes font-light">
              Here you can find all the presentations slots and their details.
            </p>
          </div>
          ...
          <div class="lowerDiv pt-10 lg:w-4/5">
            ...
            <v-row v-show="!isLoading" class="pt-5">
              <v-col v-for="file in filesInfo" :key="file.id" cols="12">
                ...
                >
                  <v-card-subtitle class="titleDes"
                    >{{ file.createdAt }}
                  </v-card-subtitle>
                  <v-card-text class="pb-0">
                    <div
                      class="flex title justify-between flex-col gap-2 md:flex-row"
                    >
                      <h1
                        class="text-base text-ellipsis max-w-lg md:w-3/5 whitespace-nowrap overflow-hidden"
                      >
                        {{ file.originalName }}
                      </h1>
                      <a
                  ...
                  <v-card-actions>
                    <v-btn
                      ...
                      size="small"
                      variant="text"
                      class="text-red-400 title"
                      text="Delete"
                      @click="deleteFile(file.id, file.autogeneratedName)"
                    ></v-btn>
                    <v-btn
                      v-if="
                        file.autogeneratedName.includes('.pdf') &&
                        sourcePDF === ''
                      "
                      color="orange"
                      size="small"
                      variant="text"
                      :class="isAdmin && isLecturer ? ' ml-4 title' : ' title'"
                      text="View File"
                      @click="viewFile(file.autogeneratedName)"
                    ></v-btn>
                    <v-btn
                      v-if="
                        file.autogeneratedName.includes('.pdf') &&
                        sourcePDF !== '' &&
                        sourcePDF.includes(file.autogeneratedName)
                      "
                      color="pink"
                      size="small"
                      variant="text"
...
const isLecturer = payload.value.user_type === "lecturer";
...
const viewFile = (autogeneratedName) => {
  sourcePDF.value = `${apiUrl}/files/${autogeneratedName}`;
};
...
const uploadFiles = async () => {
  ...
  formData.append("submissionType", "presentationFiles");
  try {
...
onMounted(async () => {
  ...
  try {
    const responseMain = await axios.get(
      `${apiUrl}/api/files/presentationFiles`,
      {
        headers: {
...
const deleteFile = async (id, autogeneratedName) => {
  ...
  try {
    const response = await axios.delete(`${apiUrl}/api/deleteFile/${id}`, {
      ...
      data: {
        collectionName: "presentationFiles",
